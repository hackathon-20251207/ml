...
#         """
#         clip = np.array(x).astype(np.float32) / 255.0
#         # Для MViTv2-small-32-2 нужен 6D-тензор: [batch, channels, frames, temporal_crops, height, width]
#         clip = rearrange(clip, "t h w c -> 1 c t 1 h w")

#         prediction = self.model([self.output_name], {self.input_name: clip})[0]
#         prediction = self.softmax(prediction)
#         prediction = np.squeeze(prediction)
#         topk_labels = prediction.argsort()[-self.topk:][::-1]
#         topk_confidence = prediction[topk_labels]
